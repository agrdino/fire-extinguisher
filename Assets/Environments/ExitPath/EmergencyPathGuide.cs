...
namespace _Scripts.Controller
{
    ...
    public sealed class EmergencyPathGuide : MonoBehaviour
    {
        ...
        [FormerlySerializedAs("_exitPathEndOffset")]
        [SerializeField, Min(0f)] private float _targetPathEndOffset = 0.25f;
        ...
        public void Initialize(Transform playerPositionSource, Transform playerGroundReference)
        {
        ...
        private void LateUpdate()
        {
            ...
            if (Time.unscaledTime >= _nextRefreshTime) RefreshPath();
        ...
        private void RefreshPath()
        {
            ...
            if (_lineRenderer == null || _target == null || _playerPositionSource == null)
            {
                HideGuide();
                return;
            }
            ...
            if (!NavMesh.SamplePosition(playerPosition, out NavMeshHit startHit, _sampleRadius, NavMesh.AllAreas)
                || !NavMesh.SamplePosition(_target.position, out NavMeshHit endHit, _sampleRadius, NavMesh.AllAreas)
            ...
            for (int index = 0; index < corners.Length; index++) _lineRenderer.SetPosition(index, corners[corners.Length - 1 - index] + Vector3.up * _groundOffset);

            Vector3 pathStart = Vector3.MoveTowards(corners[0], corners[1], _playerPathStartOffset);
            Vector3 pathEnd = Vector3.MoveTowards(corners[corners.Length - 1], corners[corners.Length - 2], _targetPathEndOffset);
        ...
        private void UpdatePlayerGroundMarker(Vector3 groundPosition, Vector3 firstCorner)
        {
            ...
            if (direction.sqrMagnitude > Mathf.Epsilon) _markerTargetYaw = Mathf.Atan2(direction.x, direction.z) * Mathf.Rad2Deg;
        ...
        private void SmoothPlayerGroundMarker()
        {
            ...
            _playerGroundMarker.position = Vector3.SmoothDamp(_playerGroundMarker.position, _markerTargetPosition, ref _markerPositionVelocity, _markerPositionSmoothTime, Mathf.Infinity, deltaTime);
            float yaw = Mathf.MoveTowardsAngle(_playerGroundMarker.eulerAngles.y, _markerTargetYaw, _markerRotationSpeed * deltaTime);
        ...
        private void SetMarkerVisible(bool isVisible)
        {
            ...
            if (_playerGroundMarker != null) _playerGroundMarker.gameObject.SetActive(isVisible);
        }
